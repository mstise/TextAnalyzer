...
def ner_retriever(entity_path=paths.get_external_disk_path(), annotated_path="/home/duper/Desktop/tmp2"):
    correct = 0
    ...
    for filename in os.listdir(annotated_path):
        ...
        for line in open(annotated_path + "/" + filename):
            ...
            mention_not_found += len(mentions)
    recall = 1 - (ground_truth_not_found / (correct + ground_truth_not_found))
    precision = 1 - (mention_not_found / (correct + mention_not_found))
